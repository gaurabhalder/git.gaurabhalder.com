...
class BlogController extends Controller
{
    ...
    public function store(StoreBlogRequest $request)
    {
        ...
        foreach ($request->input('blog_image', []) as $file) {
            $blog->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('blog_image');
        }
    ...
    public function update(UpdateBlogRequest $request, Blog $blog)
    {
        ...
        if (count($blog->blog_image) > 0) {
            foreach ($blog->blog_image as $media) {
                if (!in_array($media->file_name, $request->input('blog_image', []))) {
                    $media->delete();
                }
            }
        }
        $media = $blog->blog_image->pluck('file_name')->toArray();
        foreach ($request->input('blog_image', []) as $file) {
